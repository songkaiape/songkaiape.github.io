...
这给了我们一个: $ O(N^2) $的DP算法，使用空间是: $ O(N^2) $.

# 暴力解决法——$ O(N^3) $：
最简单的暴力解决办法就是找到所有可能的字符子串然后确定它是不是回文字符串。总共有C(n,2)种字符子串(除了单个字符的字符串)

由于判断每个子串需要O(N)时间，这个算法的时间复杂度就是$ O(N^3) $.

# 动态规划解法，$ O(N^2) $时间和$ O(N^2) $空间
...
因为扩展一个回文字符串需要花费O(N)的时间，总共的时间复杂度就是$ O(N^2) $。
